...
export function selectEvidence(evidence: EvidenceSource[], query: string, limit = 4): EvidenceSource[] {
  ...
  if (!scored.length) return evidence.slice(0, limit);
  return scored.slice(0, limit).map(({ source }) => source);
}

// Consequential questions are answered by the user, never by the model, even
// when evidence exists. This guard is the reasoning-layer backstop; the browser
// layer pauses these categories anyway.
const sensitivePatterns: RegExp[] = [
  /salary|compensation|expected pay|wage|expected income|ctc|bonus|equity/i,
  /legally authorized|work authorization|authorized to work|sponsorship|visa sponsorship|right to work/i,
  /penalty of perjury|under oath|legal declaration|legally binding/i,
  /ethnicity|religion|religious|gender|marital|disability|sexual orientation|age\b/i,
  /did you use ai|ai tools to (complete|write|prepare)|chatgpt|generated by ai/i,
];

export function sensitiveGuard(question: string): ProposedAnswer | null {
  if (!sensitivePatterns.some((pattern) => pattern.test(question))) return null;
  return { status: "needs_user", answer: "", sourceIds: [], reason: "This question is sensitive (compensation, authorization, legal, demographic, or AI-use disclosure) and must be answered by the user." };
}
...
export class GatewayReasoningModel implements ReasoningModel {
  ...
  // Empty evidence is a legitimate refusal, not a failure; only provider or
  // parse problems throw.
  async resolveAnswerStrict(request: AnswerRequest): Promise<ProposedAnswer> {
    const guarded = sensitiveGuard(request.question);
  ...
  async resolveAnswer({ question, evidence }: AnswerRequest): Promise<ProposedAnswer> {
    ...
    const guarded = sensitiveGuard(question);
    if (guarded) return guarded;
